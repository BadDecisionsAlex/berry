...
bclass *
be_newclass( bvm * vm, bstring * name, bclass * super )
{
...
void
be_member_bind( bclass * c, bstring * name )
{
  bmap *   map = c->members;
...
void
be_method_bind( bvm * vm, bclass * c, bstring * name, bproto * p )
{
...
void
be_prim_method_bind( bclass * c, bstring * name, bntvfunc f )
{
  bvalue * m = be_map_insertstr( c->members, name, NULL );
...
binstance *
instance_member( binstance * obj, bstring * name, bvalue * dst )
{
  while ( obj )
    {
...
int
be_class_newobj( bvm * vm, bclass * c, bvalue * argv, int argc )
{
  bvalue      init;
  ...
  if ( obj && var_type( &init ) != MT_VARIABLE )
    {
      ...
      bvalue * reg = argv + 1;
       /* copy argv */
      for (; argc > 0; --argc ) reg[argc] = argv[argc - 1];
...
int
be_instance_member( binstance * obj, bstring * name, bvalue * dst )
{
  int type;
...
int
be_instance_setmember( binstance * obj, bstring * name, bvalue * src )
{
  bvalue v;
