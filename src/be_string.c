...
#define be_define_const_str( _name, _s, _hash, _extra, _len, _next )            \
  ...
  {                                                                             \
    .next   = (bgcobject *) _next, .type                           = BE_STRING, \
    .marked = GC_CONST,                                                         \
    .extra  = _extra,              .slen                           = _len,      \
    .hash   = _hash,               .s                              = _s         \
  }
...
int
be_eqstr( bstring * s1, bstring * s2 )
{
  int slen;
...
/* FNV-1a Hash */
uint32_t
str_hash( const char * str, size_t len )
{
  uint32_t hash = 2166136261u;

  while ( len-- ) hash = ( hash ^ *str++ ) * 16777619u;
...
static bstring *
find_conststr( const char * str, size_t len )
{
...
static bstring *
newshortstr( bvm * vm, const char * str, size_t len )
{
  bstring *  s;
  ...
  *list = s;
  vm->strtab->count++;
  if ( vm->strtab->count > size << 2 ) resize( vm, size << 1 );

  return( s );
}

static bstring *
newlongstr( bvm * vm, const char * str, size_t len )
{
  bstring *  s;
  blstring * ls;
...
bstring *
be_newstrn( bvm * vm, const char * str, size_t len )
{
...
be_gcstrtab( bvm * vm )
{
  ...
  if ( strtab->count < size >> 2 && size > 8 ) resize( vm, size >> 1 );
}
