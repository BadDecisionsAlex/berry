...
be_initupvals( bvm * vm, bclosure * cl )
{
  ...
  for ( i = 0; i < count; ++i )
    {
      bvalue * ref;
      if ( desc->instack ) ref = stack + desc->idx;
      else ref = superuv[desc->idx]->value;
      desc++;
...
bupval *
be_findupval( bvm * vm, bvalue * level )
{
  bupval * node = vm->upvalist;

  while ( node != NULL && node->value > level ) node = node->u.next;
...
void
be_upvals_close( bvm * vm, bvalue * level )
{
  ...
  if ( cl )
    {
      ...
      while ( nupval-- ) cl->upvals[nupval] = NULL;
    }
...
bntvclos *
be_newntvclosure( bvm * vm, bntvfunc cf, int nupvals )
{
