...
static int
codeinst( bfuncinfo * finfo, binstruction ins )
{
...
static int
codeABC( bfuncinfo * finfo, bopcode op, int a, int b, int c )
{
  return( codeinst(
...
static void
free_expreg( bfuncinfo * finfo, bexpdesc * e )
{
   /* release temporary register */
  if ( e->type == ETREG ) be_code_freeregs( finfo, 1 );
}

static void
allocstack( bfuncinfo * finfo, int count )
{
  int nstack = finfo->freereg + count;

  if ( nstack > finfo->nstack ) finfo->nstack = (bbyte) nstack;
}

int
be_code_allocregs( bfuncinfo * finfo, int count )
{
...
static void
setjump( bfuncinfo * finfo, int pc, int dst )
{
...
static int
isjumpbool( bfuncinfo * finfo, int pc )
{
  binstruction * p  = be_vector_at( &finfo->code, pc );
  bopcode        op = IGET_OP( *p );

  if ( op == OP_JMPT || op == OP_JMPF ) return( 1 );

  return( 0 );
}

static int
get_jump( bfuncinfo * finfo, int pc )
{
...
static void
patchlistaux( bfuncinfo * finfo, int list, int vtarget, int dtarget )
{
  while ( list != NO_JUMP )
    {
...
static int
appendjump( bfuncinfo * finfo, bopcode op, bexpdesc * e )
{
  int j, jpc = finfo->jpc;
...
void
be_code_jumpto( bfuncinfo * finfo, int dst )
{
  be_code_patchlist( finfo, be_code_jump( finfo ), dst );
}

void
be_code_jumpbool( bfuncinfo * finfo, bexpdesc * e, int jture )
{
...
/* connect jump */
void
be_code_conjump( bfuncinfo * finfo, int * list, int jmp )
{
  if ( jmp == NO_JUMP ) return;
...
void
be_code_patchlist( bfuncinfo * finfo, int list, int dst )
{
  if ( dst == finfo->pc ) be_code_patchjump( finfo, list );
  else patchlistaux( finfo, list, dst, dst );
}

void
be_code_patchjump( bfuncinfo * finfo, int jmp )
{
...
static int
newconst( bfuncinfo * finfo, bvalue * k )
{
  ...
  if ( k == NULL ) var_setnil( &finfo->proto->ktab[idx] );

  return( idx );
}

static int
findconst( bfuncinfo * finfo, bexpdesc * e )
{
  ...
  for ( i = 0; i < count; ++i )
    {
      ...
      switch ( e->type )
        {
        case ETINT:
          if ( var_isint( k ) && k->v.i == e->v.i ) return( i );

          break;

        case ETREAL:
          if ( var_isreal( k ) && k->v.r == e->v.r ) return( i );
...
static int
exp2const( bfuncinfo * finfo, bexpdesc * e )
{
...
static void
free_suffix( bfuncinfo * finfo, bexpdesc * e )
{
  int idx    = e->v.ss.idx;
  int nlocal = be_list_count( finfo->local );

  if ( !isK( idx ) && idx > nlocal ) be_code_freeregs( finfo, 1 );
...
static int
code_suffix( bfuncinfo * finfo, bopcode op, bexpdesc * e, int dst )
{
  free_suffix( finfo, e ); /* free temporary register */
  if ( dst > finfo->freereg ) dst = finfo->freereg;
...
static void
code_closure( bfuncinfo * finfo, bproto * proto, int dst )
{
...
static int
var2reg( bfuncinfo * finfo, bexpdesc * e, int dst )
{
  switch ( e->type )
    {
    case ETINT:
      if ( e->v.i >= IsBx_MIN && e->v.i <= IsBx_MAX )
        {
          codeABx(
            finfo, OP_LDINT, dst, var_toidx( e ) + IsBx_MAX
                 );
        }
      else
        {
...
static int
exp2reg( bfuncinfo * finfo, bexpdesc * e, int dst )
{
...
static int
codedestreg( bfuncinfo * finfo, bexpdesc * e1, bexpdesc * e2 )
{
...
static void
binryexp( bfuncinfo * finfo, bopcode op, bexpdesc * e1, bexpdesc * e2 )
{
...
void
be_code_prebinop( bfuncinfo * finfo, int op, bexpdesc * e )
{
  switch ( op )
    {
    case OptAnd:
...
void
be_code_binop( bfuncinfo * finfo, int op, bexpdesc * e1, bexpdesc * e2 )
{
  switch ( op )
    {
    case OptAnd:
...
static int
code_neg( bfuncinfo * finfo, bexpdesc * e )
{
  switch ( e->type )
    {
    case ETINT:
...
static int
code_flip( bfuncinfo * finfo, bexpdesc * e )
{
  switch ( e->type )
    {
    case ETINT:
...
int
be_code_unop( bfuncinfo * finfo, int op, bexpdesc * e )
{
  switch ( op )
    {
    case OptNot:
...
static void
setsupvar( bfuncinfo * finfo, bopcode op, bexpdesc * e1, bexpdesc * e2 )
{
...
static void
setsfxvar( bfuncinfo * finfo, bopcode op, bexpdesc * e1, bexpdesc * e2 )
{
  int obj = e1->v.ss.obj;
  int src = exp2anyreg( finfo, e2 );
...
int
be_code_setvar( bfuncinfo * finfo, bexpdesc * e1, bexpdesc * e2 )
{
  switch ( e1->type )
    {
    case ETLOCAL:
    { /* It can't be ETREG. */
      ...
      if ( e1->v.idx != src ) code_move( finfo, e1->v.idx, src );
      break;
    }
...
int
be_code_nextreg( bfuncinfo * finfo, bexpdesc * e )
{
...
int
be_code_getmethod( bfuncinfo * finfo, bexpdesc * e )
{
  int dst = finfo->freereg;
...
void
be_code_call( bfuncinfo * finfo, int base, int argc )
{
...
void
be_code_closure( bfuncinfo * finfo, bexpdesc * e, bproto * proto )
{
...
void
be_code_close( bfuncinfo * finfo, int isret )
{
  bblockinfo * binfo = finfo->binfo;

  if ( isret ) /* in 'return' */
    {
      while ( binfo && !binfo->hasupval ) binfo = binfo->prev;
      if ( binfo ) codeABC( finfo, OP_CLOSE, 0, 0, 0 );
    }
  else if ( binfo->prev ) /* leave block */
    {
      if ( binfo->hasupval )
        {
          codeABC(
            finfo, OP_CLOSE, binfo->nactlocals, 0, 0
                 );
        }
    }
}
...
void
be_code_ret( bfuncinfo * finfo, bexpdesc * e )
{
  if ( e == NULL )
    {
...
void
be_code_member( bfuncinfo * finfo, bexpdesc * c, bexpdesc * k )
{
...
void
be_code_index( bfuncinfo * finfo, bexpdesc * c, bexpdesc * k )
{
  c->v.ss.obj = exp2anyreg( finfo, c );
  c->v.ss.idx = exp2anyreg( finfo, k );
  c->v.ss.tt  = c->type;
...
bvalue *
be_code_localobject( bfuncinfo * finfo, int dst )
{
...
void
be_code_setsuper( bfuncinfo * finfo, bexpdesc * c, bexpdesc * s )
{
...
void
be_code_import( bfuncinfo * finfo, bexpdesc * m, bexpdesc * v )
{
