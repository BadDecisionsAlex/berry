...
static void
class_init( bvm * vm, bclass * c, const bnfuncinfo * lib )
{
  while ( lib->name )
    {
...
void
be_regfunc( bvm * vm, const char * name, bntvfunc f )
{
...
int
be_isfunction( bvm * vm, int index )
{
  bvalue * v = index2value( vm, index );
...
int
be_isinstance( bvm * vm, int index )
{
  bvalue * v = index2value( vm, index );
...
void
be_moveto( bvm * vm, int from, int to )
{
...
const char *
be_pushfstring( bvm * vm, const char * format, ... )
{
  const char * s;
  va_list      arg_ptr;
...
void
be_pushntvfunction( bvm * vm, bntvfunc f )
{
  bvalue * top = be_incrtop( vm );
...
void
be_pushclass( bvm * vm, const char * name, const bnfuncinfo * lib )
{
  bstring * s;
  bclass *  c;
...
void
be_setmember( bvm * vm, int index, const char * k )
{
...
void
be_getmember( bvm * vm, int index, const char * k )
{
  bvalue * o   = index2value( vm, index );
  bvalue * top = be_incrtop( vm );
...
void
be_getupval( bvm * vm, int index, int pos )
{
...
void
be_setupval( bvm * vm, int index, int pos )
{
  bvalue * f = index2value( vm, index );
  bvalue * v = index2value( vm, -1 );
...
void
be_data_append( bvm * vm, int index )
{
  bvalue * o = index2value( vm, index );
  bvalue * v = index2value( vm, -1 );
...
int
be_data_insert( bvm * vm, int index )
{
  bvalue * o = index2value( vm, index );
  bvalue * k = index2value( vm, -2 );
  bvalue * v = index2value( vm, -1 );
...
int
be_data_remove( bvm * vm, int index )
{
  bvalue * o = index2value( vm, index );
  bvalue * k = index2value( vm, -1 );
...
void
be_data_resize( bvm * vm, int index )
{
  bvalue * o = index2value( vm, index );
  bvalue * v = index2value( vm, -1 );
...
int
be_iter_hasnext( bvm * vm, int index )
{
  bvalue * o = index2value( vm, index );
...
int
be_refcontains( bvm * vm, int index )
{
  bvalue *     v   = index2value( vm, index );
