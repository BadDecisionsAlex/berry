...
#if BE_DEBUG
  #define FULL_VERSION    "LSD " BERRY_VERSION " (debug)"
#else
...
analysis_args( bvm * vm )
{
  ...
  if ( args & arg_i ) be_writestring( repl_prelude );
  if ( be_top( vm ) > 0 )
    {
      int res = dofile( vm );
...
main( int argc, char * argv[] )
{
  ...
  be_loadlibs( vm );
  for ( i = 1; i < argc; ++i ) be_pushstring( vm, argv[i] );
