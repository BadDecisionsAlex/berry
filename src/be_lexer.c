...
void
be_lexerror( blexer * lexer, const char * msg )
{
  bvm * vm = lexer->vm;
...
static int
check2hex( blexer * lexer, int c )
{
  c = char2hex( c );
  if ( c < 0 ) be_lexerror( lexer, "hexadecimal digit expected." );

  return( c );
}

static int
read_hex( blexer * lexer, const char * src )
{
...
static int
read_oct( blexer * lexer, const char * src )
{
  int i, c = 0;
...
static btokentype
scan_assign( blexer * lexer, btokentype is, btokentype not )
{
  next( lexer );
  ...
  for (;; )
    {
      switch ( lgetc( lexer ))
        ...
        case EOS:
          ...
          return( TokenEOS ); /* end of source stream */
...
int
be_lexer_scan_next( blexer * lexer )
{
  btokentype type;
...
const char *
be_token2str( bvm * vm, btoken * token )
{
  switch ( token->type )
    {
    case TokenString:
...
const char *
be_tokentype2str( btokentype type )
{
