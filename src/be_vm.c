...
#define RKB( i )                          \
  (( isKB( i ) ? curcl( vm )->proto->ktab \
               : vm->reg ) + KR2idx( IGET_RKB( i )))
#define RKC( i )                          \
  (( isKC( i ) ? curcl( vm )->proto->ktab \
...
static void
binop_error( bvm * vm, const char * op, bvalue * a, bvalue * b )
{
  vm_error(
    vm,
...
static void
unop_error( bvm * vm, const char * op, bvalue * a )
{
  vm_error(
    vm,
...
static void
precall( bvm * vm, bvalue * func, int nstack, int mode )
{
  bcallframe * cf;
...
static void
obj_method( bvm * vm, bvalue * o, bstring * attr )
{
...
static int
obj_attribute( bvm * vm, bvalue * o, bstring * attr, bvalue * dst )
{
  binstance * obj  = var_toobj( o );
...
static void
object_unop( bvm * vm, const char * op, binstruction ins, bvalue * src )
{
  bvalue * top = vm->top;
...
static void
i_ldnil( bvm * vm, binstruction ins )
{
  var_setnil( RA( ins ));
}

static void
i_ldbool( bvm * vm, binstruction ins )
{
  bvalue * v = RA( ins );
...
static void
i_ldint( bvm * vm, binstruction ins )
{
  bvalue * v = RA( ins );
...
static void
i_getglobal( bvm * vm, binstruction ins )
{
  bvalue * v   = RA( ins );
  int      idx = IGET_Bx( ins );
...
static void
i_setglobal( bvm * vm, binstruction ins )
{
  bvalue * v   = RA( ins );
  int      idx = IGET_Bx( ins );
...
static void
i_getupval( bvm * vm, binstruction ins )
{
  bvalue * v   = RA( ins );
  int      idx = IGET_Bx( ins );
...
static void
i_setupval( bvm * vm, binstruction ins )
{
  bvalue * v   = RA( ins );
  int      idx = IGET_Bx( ins );
...
static void
i_ldconst( bvm * vm, binstruction ins )
{
  bvalue * dst = RA( ins );
...
static void
i_move( bvm * vm, binstruction ins )
{
  bvalue * dst = RA( ins );
  ...
  if ( var_isint( a ) && var_isint( b ))
    {
      ...
      if ( y == 0 ) vm_error( vm, "division by zero" );
      var_setint( dst, x / y );
    }
  else if ( var_isnumber( a ) && var_isnumber( b ))
    {
      breal x = var2real( a ), y = var2real( b );
      if ( y == cast( breal, 0 )) vm_error( vm, "division by zero" );
      var_setreal( dst, x / y );
    }
  else if ( var_isinstance( a ))
    {
...
static void
i_neg( bvm * vm, binstruction ins )
{
  bvalue * dst = RA( ins ), * a = RKB( ins );

  if ( var_isint( a )) var_setint( dst, -a->v.i );
  else if ( var_isreal( a )) var_setreal( dst, -a->v.r );
  else if ( var_isinstance( a )) object_unop( vm, "-*", ins, a );
  else unop_error( vm, "-", a );
}

static void
i_not( bvm * vm, binstruction ins )
{
  bvalue * dst = RA( ins ), * a = RKB( ins );
...
define_function( i_lt, relop_block( < )) define_function(
  i_le,
  relop_block( <= )
                                                        ) define_function(
  i_gt,
...
define_function( i_eq, equal_block( == )) define_function(
  i_ne,
  equal_block( != )
                                                         )
define_function( i_and, bitwise_block( & )) define_function(
  i_or,
  bitwise_block( | )
                                                           )
define_function( i_xor, bitwise_block (^)) define_function(
  i_shl,
  bitwise_block(<<)
                                                          )
define_function( i_shr, bitwise_block( >> )) static void
i_range( bvm * vm, binstruction ins )
{
...
static void
i_jump( bvm * vm, binstruction ins )
{
  vm->ip += IGET_sBx( ins );
}

static void
i_jumptrue( bvm * vm, binstruction ins )
{
  if ( be_value2bool( vm, RA( ins ))) vm->ip += IGET_sBx( ins );
}

static void
i_jumpfalse( bvm * vm, binstruction ins )
{
  if ( !be_value2bool( vm, RA( ins ))) vm->ip += IGET_sBx( ins );
}

static void
i_return( bvm * vm, binstruction ins )
{
  bcallframe * cf  = vm->cf;
  bvalue *     ret = vm->cf->func;

   /* copy return value */
  if ( IGET_RA( ins )) *ret = *RKB( ins );
  else var_setnil( ret );
  vm->reg = cf->reg;
  vm->top = cf->top;
  vm->ip  = cf->ip;
  ...
  else vm->cf = be_stack_top( &vm->callstack );
}

static void
i_call( bvm * vm, binstruction ins )
{
  bvalue * var = RA( ins );
...
static void
i_closure( bvm * vm, binstruction ins )
{
  bvalue *   reg = RA( ins );
...
static void
i_getmember( bvm * vm, binstruction ins )
{
...
static void
i_getmethod( bvm * vm, binstruction ins )
{
  bvalue * a = RA( ins ), * b = RKB( ins ), * c = RKC( ins );
...
static void
i_setmember( bvm * vm, binstruction ins )
{
  bvalue * a = RA( ins ), * b = RKB( ins ), * c = RKC( ins );
...
static void
i_getindex( bvm * vm, binstruction ins )
{
  bvalue * b = RKB( ins ), * c = RKC( ins );
...
static void
i_setindex( bvm * vm, binstruction ins )
{
  bvalue * a = RA( ins ), * b = RKB( ins ), * c = RKC( ins );
...
static void
i_setsuper( bvm * vm, binstruction ins )
{
...
static void
i_close( bvm * vm, binstruction ins )
{
  be_upvals_close( vm, RA( ins ));
}

static void
i_import( bvm * vm, binstruction ins )
{
...
newframe: for (;; )
    {
      ...
      switch ( IGET_OP( ins ))
        ...
        default:
retpoint: if ( vm->cf == NULL )
            {
              bstack * cs = &vm->callstack;
              if ( !be_stack_isempty( cs )) vm->cf = be_stack_top( cs );
...
static void
do_closure( bvm * vm, bvalue * reg, int argc )
{
  bvalue * v, * end;
...
static void
do_ntvclos( bvm * vm, bvalue * reg, int argc )
{
...
static void
do_ntvfunc( bvm * vm, bvalue * reg, int argc )
{
...
static void
do_class( bvm * vm, bvalue * reg, int argc )
{
...
void
be_dofunc( bvm * vm, bvalue * v, int argc )
{
  be_gc_setpause( vm, 1 );
  switch ( var_type( v ))
    {
