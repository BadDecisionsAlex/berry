...
/*
   A fake variable is added at the end of each definition to each a global
   scope semicolon.
 */
#define VM_CAT( x, y ) _VM_CAT( x, y )
#define _VM_CAT( x, y ) x ## y
...
#define define_function( name, block ) \
  ...
  {                                    \
    block                              \
  } char VM_CAT( __FAKE_, __LINE__ ) __attribute__ (( unused )) = 0
