...
bvalue *
be_list_index( blist * list, int index )
{
...
bvalue *
be_list_append( blist * list, bvalue * value )
{
  bvalue * slot;
...
bvalue *
be_list_insert( blist * list, int index, bvalue * value )
{
  int      i;
  bvalue * data;
...
int
be_list_remove( blist * list, int index )
{
  int      i;
  bvalue * data;

  if ( index < 0 ) index = list->count + index;
  if ( index < 0 || index >= list->count ) return( bfalse );

  data = list->data;
  for ( i = index; i < list->count; ++i ) data[i] = data[i + 1];
  --list->count;
...
void
be_list_resize( blist * list, int count )
{
  if ( count != list->count )
    {
