...
static void
match_token( bparser * parser, btokentype type )
{
...
static void
match_notoken( bparser * parser, btokentype type )
{
  if ( next_type( parser ) == type )
    {
      push_error(
        parser,
        "expected statement before '%s'",
        token2str( parser )
                );
    }
}
...
static void
check_symbol( bparser * parser, bexpdesc * e )
{
...
static void
check_var( bparser * parser, bexpdesc * e )
{
...
static void
begin_block( bfuncinfo * finfo, bblockinfo * binfo, int isloop )
{
...
static void
begin_func( bparser * parser, bfuncinfo * finfo, bblockinfo * binfo )
{
  bvm *    vm    = parser->vm;
...
static btokentype
get_assign_op( bparser * parser )
{
  btokentype op = next_type( parser );

  if ( op >= OptAssign && op <= OptRsfAssign ) return( op );

  return( OP_NOT_ASSIGN );
}

static void
init_exp( bexpdesc * e, exptype_t type, bint i )
{
  e->type = (bbyte) type;
  e->t    = NO_JUMP;
  e->f    = NO_JUMP;
...
static void
string_protect( bparser * parser, bstring * s )
{
  bvm * vm = parser->vm;
...
static int
find_localvar( bfuncinfo * finfo, bstring * s )
{
...
static int
new_localvar( bfuncinfo * finfo, bstring * s )
{
...
static int
find_upval( bfuncinfo * finfo, bstring * s )
{
...
static void
mark_upval( bfuncinfo * finfo, int level )
{
  bblockinfo * binfo = finfo->prev->binfo;

  while ( binfo->nactlocals > level ) binfo = binfo->prev;
  binfo->hasupval = 1;
}

static int
new_upval( bfuncinfo * finfo, bstring * name, bexpdesc * var )
{
  int      index;
  bvalue * desc;
...
static void
new_var( bparser * parser, bstring * name, bexpdesc * var )
{
  bfuncinfo * finfo = parser->finfo;
...
static void
new_class( bparser * parser, bstring * name, bclass * c, bexpdesc * e )
{
  bvalue *    var;
  bfuncinfo * finfo = parser->finfo;
...
static int
singlevaraux( bvm * vm, bfuncinfo * finfo, bstring * s, bexpdesc * var )
{
  if ( finfo == NULL )
    {
      return( ETVOID );
...
static void
singlevar( bparser * parser, bexpdesc * var )
{
...
static bproto *
funcbody( bparser * parser, bstring * name, int ismethod )
{
  bfuncinfo  finfo;
  bblockinfo binfo;
...
/* anonymous function */
static void
anon_func( bparser * parser, bexpdesc * e )
{
  bproto * proto;
...
static void
new_primtype( bparser * parser, const char * type, bexpdesc * e )
{
  int         idx;
  bvm *       vm    = parser->vm;
  bfuncinfo * finfo = parser->finfo;
...
static void
list_nextmember( bparser * parser, bexpdesc * l )
{
  bexpdesc    e, key, v = *l;
  bfuncinfo * finfo = parser->finfo;
  int         base;
...
static void
map_nextmember( bparser * parser, bexpdesc * l )
{
  bexpdesc    e, key, v = *l;
  bfuncinfo * finfo = parser->finfo;
  int         base;
...
static void
list_expr( bparser * parser, bexpdesc * e )
{
   /* '[' {expr ','} [expr] ']' */
...
static void
map_expr( bparser * parser, bexpdesc * e )
{
   /* '{' {expr ':' expr ','} [expr ':' expr] '}' */
...
static int
exprlist( bparser * parser, bexpdesc * e )
{
  bfuncinfo * finfo = parser->finfo;
  int         n     = 1;
...
static void
call_expr( bparser * parser, bexpdesc * e )
{
  bexpdesc    args;
  bfuncinfo * finfo = parser->finfo;
  ...
  if ( ismember ) base = be_code_getmethod( finfo, e );
  else base = be_code_nextreg( finfo, e );
  scan_next_token( parser );     /* skip '(' */
  if ( next_type( parser ) != OptRBK ) argc = exprlist( parser, &args );
  match_token( parser, OptRBK ); /* skip ')' */
  argc += ismember;
...
static void
member_expr( bparser * parser, bexpdesc * e )
{
   /* . ID */
  check_var( parser, e );
  scan_next_token( parser ); /* skip '.' */
...
static void
index_expr( bparser * parser, bexpdesc * e )
{
  bexpdesc e1;
...
primary_expr( bparser * parser, bexpdesc * e )
{
  switch ( next_type( parser ) )
    ...
    case OptLBK:                 /* '(' expr ')' */
      scan_next_token( parser ); /* skip '(' */
      /* sub_expr() is more efficient because there is no need to initialize
         e. */
...
static void
suffix_expr( bparser * parser, bexpdesc * e )
{
...
/* compound assignment */
static void
compound_assign( bparser * parser, btokentype op, bexpdesc * l, bexpdesc * r )
{
  ...
  if ( op != OptAssign ) /* compound assignment */
    {
      bexpdesc e = *l;
      op = op
           < OptAndAssign ? op - OptAddAssign + OptAdd : op - OptAndAssign
           + OptBitAnd;
 ...
 */
static void
sub_expr( bparser * parser, bexpdesc * e, int prio )
{
  bfuncinfo * finfo = parser->finfo;
...
static void
expr( bparser * parser, bexpdesc * e )
{
  init_exp( e, ETVOID, 0 );
  sub_expr( parser, e, ASSIGN_OP_PRIO );
}
...
static void
condition_block( bparser * parser, int * jmp )
{
  bfuncinfo * finfo = parser->finfo;
  int         br    = cond_stmt( parser );

  block( parser );
  if ( next_type( parser ) == KeyElif || next_type( parser ) == KeyElse )
    {
      be_code_conjump(
        finfo,
        jmp,
...
static void
for_itvar( bparser * parser, bexpdesc * e )
{
  if ( next_type( parser ) == TokenId )
    {
...
static void
for_init( bparser * parser, bexpdesc * v )
{
  bexpdesc    e;
  bstring *   s;
  bvm *       vm    = parser->vm;
 ...
 */
static void
for_iter( bparser * parser, bexpdesc * v, bexpdesc * it )
{
  bexpdesc    e;
  bstring *   s;
  int         brk;
...
get_loop_block( bparser * parser )
{
  ...
  while ( b != NULL && !b->isloop ) b = b->prev;

  return( b );
}

static void
break_stmt( bparser * parser )
{
  bfuncinfo *  f = parser->finfo;
...
static bstring *
func_name( bparser * parser, bexpdesc * e, int ismethod )
{
  ...
  if ( type == TokenId )
    {
      ...
      if ( !ismethod ) new_var( parser, name, e );  /* new variable */
...
static void
classvar_stmt( bparser * parser, bclass * c )
{
   /* 'var' ID {',' ID} */
...
static void
classdef_stmt( bparser * parser, bclass * c )
{
  bexpdesc  e;
  bstring * name;
  bproto *  proto;
...
static void
class_inherit( bparser * parser, bexpdesc * e )
{
...
static void
mainfunc( bparser * parser, bclosure * cl )
{
  bblockinfo binfo;
  bfuncinfo  finfo;
...
bclosure *
be_parser_source( bvm * vm, const char * fname, breader reader, void * data )
{
  bparser    parser;
