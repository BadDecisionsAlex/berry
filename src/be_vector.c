...
void
be_vector_init( bvector * vector, int size )
{
...
void
be_vector_delete( bvector * vector )
{
  be_free( vector->data );
}

void *
be_vector_at( bvector * vector, int index )
{
...
void
be_vector_append( bvector * vector, void * data )
{
  ...
  ++vector->count;
  if ( data != NULL ) memcpy( vector->end, data, size );
}

void
be_vector_remove_end( bvector * vector )
{
  if ( vector->count > 0 )
    {
      --vector->count;
...
void
be_vector_resize( bvector * vector, int count )
{
  size_t size   = vector->size;
...
void
be_vector_clear( bvector * vector )
{
  vector->count = 0;
...
/* free not used */
void *
be_vector_release( bvector * vector )
{
  if ( vector->count == 0 )
    {
      be_free( vector->data );
