...
static uint32_t
_hashcode_( int type, union bvaldata v )
{
  switch ( type )
    {
    case BE_NIL:
...
static int
eqnode( bmapnode * node, bvalue * key, uint32_t hash )
{
  bmapkey * k = key( node );
...
static bmapnode *
findprev( bmap * map, bmapnode * list, bmapnode * slot )
{
...
static bmapnode *
insert( bmap * map, bvalue * key, uint32_t hash )
{
...
static bmapnode *
find( bmap * map, bvalue * key, uint32_t hash )
{
  bmapnode * slot = hash2slot( map, hash );
...
resize( bmap * map, int size )
{
  ...
  for ( i = 0; i < size; ++i ) setnil( slots + i );
  map->size     = size;
  map->slots    = slots;
...
bvalue *
be_map_insert( bmap * map, bvalue * key, bvalue * value )
{
...
int
be_map_remove( bmap * map, bvalue * key )
{
  uint32_t   hash = hashcode( key );
  ...
  setnil( slot );
   /* add to lastfree */
  if ( map->lastfree < slot ) map->lastfree = slot;
  --map->count;
...
bvalue *
be_map_findstr( bmap * map, bstring * key )
{
  bvalue v;
...
bvalue *
be_map_insertstr( bmap * map, bstring * key, bvalue * value )
{
  bvalue v;
...
void
be_map_removestr( bmap * map, bstring * key )
{
  bvalue v;
...
bmapnode *
be_map_next( bmap * map, bmapiter * iter )
{
...
bvalue
be_map_key2value( bmapnode * node )
{
  bvalue v;
...
void
be_map_release( bvm * vm, bmap * map )
{
  (void) vm;
