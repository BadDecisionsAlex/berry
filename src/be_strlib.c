...
bstring *
be_strcat( bvm * vm, bstring * s1, bstring * s2 )
{
...
static void
pushstr( bvm * vm, const char * s, size_t len )
{
...
const char *
be_pushvfstr( bvm * vm, const char * format, va_list arg )
{
  pushstr( vm, "", 0 );
  for (;; )
    {
...
*******************************************************************/
bint
be_str2int( const char * str, const char ** endstr )
{
  int  c, sign;
  bint sum = 0;
...
*******************************************************************/
breal
be_str2real( const char * str, const char ** endstr )
{
  int   c, sign;
 ...
 **/
const char *
be_str2num( bvm * vm, const char * str )
{
  const char * sout;
...
be_strindex( bvm * vm, bstring * str, bvalue * idx )
{
  ...
  static const char *
  get_mode( const char * str, char * buf )
  {
    const char * p = str;
  ...
  static void
  mode_fixlen( char * mode, const char * lenmode )
  {
            ...
            if ( index > top )
              {
                be_pusherror(
                  ...
                  be_pushfstring(
                    vm,
                    "bad argument #%d to 'format': no value",
                    index
                                )
                            );
              }
