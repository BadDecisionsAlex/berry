...
void
be_gc_setsteprate( bvm * vm, int rate )
{
  vm->gc->steprate = (bbyte) rate;
}

void
be_gc_setpause( bvm * vm, int pause )
{
  vm->gc->pause = (bbyte) pause;
}

static bgcobject *
gc_malloc( bvm * vm, size_t size )
{
  bgcobject * obj;
...
bgcobject *
be_newgcobj( bvm * vm, int type, size_t size )
{
  bgcobject * obj;
  bgc *       gc = vm->gc;
...
bgcobject *
be_gc_newstr( bvm * vm, size_t size, int islong )
{
  bgcobject * obj;

  if ( islong ) return( be_newgcobj( vm, BE_STRING, size ));
...
void
be_gc_fix( bvm * vm, bgcobject * obj )
{
  (void) vm;
  gc_setfixed( obj );
}
...
void
be_gc_unfix( bvm * vm, bgcobject * obj )
{
  (void) vm;
  gc_clearfixed( obj );
}
...
static void
mark_list( bvm * vm, bgcobject * obj )
{
  ...
  if ( list )
    {
      ...
      gc_setdark( obj );
      for (; count--; val++ ) mark_object( vm, val->v.gc, var_type( val ));
    }
}

static void
mark_proto( bvm * vm, bgcobject * obj )
{
...
static void
mark_closure( bvm * vm, bgcobject * obj )
{
...
static void
mark_ntvclos( bvm * vm, bgcobject * obj )
{
...
static void
mark_class( bvm * vm, bgcobject * obj )
{
...
static void
mark_instance( bvm * vm, bgcobject * obj )
{
...
static void
mark_module( bvm * vm, bgcobject * obj )
{
...
static void
mark_object( bvm * vm, bgcobject * obj, int type )
{
      ...
      for ( i = 0; i < count; ++i )
        {
          ...
          if ( uv->refcnt ) --uv->refcnt;
           /* delete non-referenced closed upvalue */
          if ( uv->value == &uv->u.value && !uv->refcnt ) be_free( uv );
        }
    }
  be_free( obj );
}
  ...
  if ( f )
    {
      int       count = f->nupvals;
      bupval ** uv    = &be_ntvclos_upval( f, 0 );
      while ( count-- ) be_free( *uv++ );
    }
}

static void
free_object( bvm * vm, bgcobject * obj )
{
  (void) vm;
  switch ( obj->type )
    {
    ...
    case BE_MAP:
      be_map_delete( cast_map( obj ));
      break;

    case BE_LIST:
      be_list_delete( cast_list( obj ));
      break;
    ...
    default:
      break; /* case BE_STRING: break; */
    }
}

static void
premark_global( bvm * vm )
{
  ...
  while ( v < end )
    {
      if ( be_isgcobj( v )) gc_setgray( var_togc( v ));
      ++v;
    }
}

static void
premark_stack( bvm * vm )
{
...
static void
premark_fixed( bvm * vm )
{
  bgcobject * node = vm->gc->list;

  for (; node; node = node->next )
    if ( gc_isfixed( node ) && gc_iswhite( node )) gc_setgray( node );

}

static void
mark_unscanned( bvm * vm )
{
  bgc *       gc = vm->gc;
  bgcobject * node;

  for ( node = gc->list; node; node = node->next )
    if ( gc_isgray( node )) mark_object( vm, node, var_type( node ));

}

static void
destruct_object( bvm * vm, bgcobject * obj )
{
  if ( obj->type == BE_INSTANCE )
    {
      ...
      type = be_instance_member(
        insobj, be_newstr( vm, "deinit" ), top
                               );
...
static void
destruct_white( bvm * vm )
{
  bgcobject * node = vm->gc->list;

  while ( node )
    {
      if ( gc_iswhite( node )) destruct_object( vm, node );
      node = node->next;
    }
}
...
static void
reset_fixedlist( bvm * vm )
{
  bgc *       gc = vm->gc;
  bgcobject * node;

  for ( node = gc->fixed; node; node = node->next )
    if ( gc_isdark( node )) gc_setgray( node );
