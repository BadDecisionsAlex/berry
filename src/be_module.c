...
find_existing( bvm * vm, bntvmodule * nm )
{
  ...
  while ( node && node->native != nm ) node = node->mnext;

  return( node );
}

static void
insert_attrs( bvm * vm, bmap * table, bntvmodule * nm )
{
  size_t i;
...
static bmodule *
new_module( bvm * vm, bntvmodule * nm, bvalue * dst )
{
...
static bmodule *
load_module( bvm * vm, bntvmodule * nm, bvalue * dst )
{
  if ( nm )
    {
...
bmodule *
be_module_load( bvm * vm, bstring * path, bvalue * dst )
{
...
void
be_module_delete( bvm * vm, bmodule * module )
{
  if ( module == vm->modulelist )
  ...
  else
    {
      bmodule * prev = vm->modulelist;
       /* find previous node */
      while ( prev && prev->mnext != module ) prev = prev->mnext;
       /* remove module node from module-list */
      if ( prev ) prev->mnext = module->mnext;
    }
  be_free( module );
}
