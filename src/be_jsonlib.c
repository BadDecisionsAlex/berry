...
#include <string.h>
  ...
  static void
  json2berry( bvm * vm, const char * class )
  {
  ...
  static char *
  load_unicode( char * dst, const char * json )
  {
    int ucode = 0, i = 4;
  ...
  static const char *
  parser_string( bvm * vm, const char * json )
  {
    if ( *json == '"' )
      {
  ...
  static const char *
  parser_field( bvm * vm, const char * json )
  {
    if ( json && *json == '"' )
      {
  ...
  static const char *
  parser_object( bvm * vm, const char * json )
  {
    json = match_char( json, '{' );
    be_newmap( vm );
    if ( *json != '}' )
  ...
  static const char *
  parser_array( bvm * vm, const char * json )
  {
    json = match_char( json, '[' );
    be_newlist( vm );
    if ( *json != ']' )
...
/* parser json value */
  static const char *
  parser_value( bvm * vm, const char * json )
  {
    json = skip_space( json );
    switch ( *json )
      {
  ...
  static void
  make_indent( bvm * vm, int stridx, int indent )
  {
    if ( indent )
      {
  ...
  static void
  object_tostr( bvm * vm, int * indent, int idx, int fmt )
  {
  ...
  static void
  array_tostr( bvm * vm, int * indent, int idx, int fmt )
  {
    be_getmember( vm, idx, ".data" );
    be_pushstring( vm, fmt ? "[\n" : "[" );
    be_pushiter( vm, -2 );
  ...
  m_json_dump( bvm * vm )
  {
    ...
    if ( argc > 1 ) fmt = !strcmp( be_tostring( vm, 2 ), "format" );
    json2str( vm, &indent, 1, fmt );
    be_return( vm );
  }
