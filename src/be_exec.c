...
int
be_execprotected( bvm * vm, bpfunc f, void * data )
{
  struct blongjmp jmp;
...
int
be_protectedparser( bvm * vm, const char * fname, breader reader, void * data )
{
  int            res;
  struct pparser s;
...
static const char *
_fgets( void * data, size_t * size )
{
  struct filebuf * fb = data;

  *size = fread( fb->buf, 1, sizeof( fb->buf ), fb->fp );
  if ( *size ) return( fb->buf );

  return( NULL );
}

int
be_loadbuffer( bvm *        vm,
               const char * name,
               const char * buffer,
               size_t       length )
{
...
int
be_protectedcall( bvm * vm, bvalue * v, int argc )
{
  int          res;
  struct pcall s;
...
void
be_stack_require( bvm * vm, int count )
{
  if ( vm->top + count >= vm->stacktop ) be_stack_expansion( vm, count );
}

static void
update_callstack( bvm * vm, bvalue * oldstack )
{
...
static void
stack_resize( bvm * vm, size_t size )
{
...
void
be_stack_expansion( bvm * vm, int n )
{
