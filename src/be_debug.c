...
static void
print_inst( binstruction ins, int pc )
{
...
void
be_dprintcode( bclosure * cl )
{
  printproto( cl->proto );
}

static const char *
sourceinfo( bvm * vm, char * buf, int deepth )
{
  #if BE_RUNTIME_DEBUG_INFO
...
static void
patch_native( bvm * vm, int deepth )
{
  int          size = be_stack_count( &vm->callstack );
  bcallframe * cf   = be_vector_at( &vm->callstack, size + deepth );

  if ( deepth == -1 ) cf->ip = vm->ip;
  else cf->ip = cf[1].ip;
}
...
void
be_pusherror( bvm * vm, const char * msg )
{
  addinfo( vm, msg );
